...
+ (NSString *)getCopyRightText {
    return [NSString stringWithFormat:@"©%d Indriam Inc.",[CommonUtility getCurrentYear]];
}
...
+ (NSString *) getRandomFont {
    ...
    int index = [CommonUtility  pickRandom:[fArray count]];
    ...
    // select a font
    index = [CommonUtility  pickRandom:[fontArray count]];
    // return the selected font
    return [fontArray objectAtIndex:index];
}
#pragma mark - Number methods
+(NSString *)wordsForInt:(NSInteger)val {
    return [CommonUtility wordsForNumber:[NSNumber numberWithInt:val]];
}
...
+ (UIColor *)getColor:(UIColor *)color withStaturation:(float)staturation {
    ...
    [color getHue:&hsba[0] saturation:&hsba[1] brightness:&hsba[2] alpha:&hsba[3]];
    // return values
    return [UIColor colorWithHue:hsba[0] saturation:staturation brightness:hsba[2] alpha:hsba[3]];
}
...
+ (NSString *)getTimeInWordForTime:(NSString *)timeString {
    int valCount=[[timeString componentsSeparatedByString:@":"] count];
    if(valCount==0) {
        return nil;
    }
    if(valCount==1) {
...
+ (NSString *)convertCentsToCurrencyString:(NSInteger)cents{
    ...
    if(dollars>0){
        return [NSString stringWithFormat:@"$%d.%02d",dollars,cents];
    }
    else{
        return [NSString stringWithFormat:@"%d¢",cents];
    }
}
